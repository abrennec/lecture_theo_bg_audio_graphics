...
void main() {
  ...
  float distortion2 = pnoise(2.0 * aPosition + uFrameCount * 0.1, vec3(0));

  //testing something from https://thebookofshaders.com/11/:
  // float x;
  // float f;
  // float i = floor(x);  // integer
  // float f = fract(x);  // fraction
  // float u = f * f * (3.0 - 2.0 * f ); // custom cubic curve
  // float vivisDistortion = mix(rand(i), rand(i + 1.0), u); // using it in the interpolation
  //Didn't work, idk why
